...
def generate_qa_from_raw_data(raw_data_path,output_json_dir,cons_box_per_img):
    ...
    # 保存QA训练数据（与原逻辑一致）
    box_qa_output_path = os.path.join(output_json_dir, "zaozi_JTEQAv2_box_anno.json")
    img_qa_output_path = os.path.join(output_json_dir, "zaozi_JTEQAv2_img_anno.json")
    with open(box_qa_output_path, "w", encoding="utf-8") as f:
        json.dump(box_qa_list, f, ensure_ascii=False, indent=2)
    # img_qa_list = random.sample(img_qa_list, 50000)
...
# def process_anno(anno_text: str,) -> tuple[str, int]:
#     """
#     1. # -> <#>
#     2.### -> <###> for en
#     """
#     # 步骤1：处理每个单词
#     words = anno_text.split()
#     processed_words = []
#     for word in words:
#         # if word.count('#')>3:
#         #     print(f'debug')
...
if __name__ == "__main__":
    ...
    RAW_DATA_PATH = '/mnt/bn/ocr-doc-nas/zhuhanshen/project/TextEvaluator/hanzi_writer/synthtiger/syth_text_data_zaozi.jsonl' # 需替换为实际数据路径
