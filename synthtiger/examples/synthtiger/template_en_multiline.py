...
class SynthTigerEN(templates.Template):
    def __init__(self, config=None):
        ...
        #     **config.get("corpus", {}),
        # )
        self.corpus = components.EnglishCorpus(**config.get("corpus", {}))
    ...
    def _generate_multiple_text(self, size, bg_image):
        ...
        while len(valid_elements) < target_num_elements or len(valid_elements)==0:#确保至少有一个元素
            ...
            if current_total_retries > max_total_retries:  # 如果总体重试次数过多，就跳出循环
                # 如果已经有有效元素，就接受当前结果；否则至少创建一个元素
                if len(valid_elements) == 0:
                    # 强制创建一个元素，不进行布局和可见性检查
                    fg_color, fg_style, _, _, _ = self._generate_color()
                    element = self._generate_text_v3(fg_color, fg_style)
                    valid_elements.append(element)
                    valid_element_layers.append(element["layer"].copy())
                    # 将元素直接添加到背景
                    temp_element_image = utils.create_image(size)
                    quad_relative = valid_element_layers[0].quad - [0, 0]
                    utils.paste_image(valid_element_layers[0].image, temp_element_image, quad_relative)
                    current_bg = utils.blend_image(temp_element_image, current_bg, mode="normal")
                break

     
            retry_count = 0
            while retry_count <= max_retries:  # 修改为更清晰的重试逻辑
